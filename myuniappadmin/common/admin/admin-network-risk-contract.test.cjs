...
test('administrator challenge accepts only its path and shares the bounded failure flow', () => {
	...
	const gate = source('../fornted/common/shared-auth/risk-challenge-failed-gate.vue')
	const core = source('../fornted/common/shared-auth/risk-challenge-state-machine.js')
...
test('administrator WebRTC verification uses generation-scoped background work and an ephemeral Android bridge', () => {
	...
	const androidVerification = source('common/admin/admin-webrtc-verification-android.js')
	const h5Verification = source('common/admin/admin-webrtc-verification-h5.js')
	const core = source('../fornted/common/shared-auth/webrtc-verification-core.js')
	const h5Probe = source('../fornted/common/shared-auth/h5-webrtc-probe.js')
	const http = source('common/admin/admin-http.js')
	const androidProbe = source('../fornted/common/shared-auth/android-webrtc-background-probe.js')
	const diagnostics = source('../fornted/common/shared-auth/webrtc-diagnostics.js')
	...
	const ordinaryCryptoInterface = source('../fornted/uni_modules/ait-webrtc-crypto/utssdk/interface.uts')
	const ordinaryCryptoAndroid = source('../fornted/uni_modules/ait-webrtc-crypto/utssdk/app-android/index.uts')
	...
	assert.match(verification, /createWebRtcDiagnosticLogger\(\s*'admin-flow',\s*WEBRTC_DIAGNOSTICS_ENABLED\s*\)/)
	...
	assert.match(verification, /collectAdminH5VerificationIps/)
	assert.doesNotMatch(verification, /collectBrowserWebRtcIps/)
	assert.match(h5Verification, /collectH5WebRtcIps/)
	assert.doesNotMatch(
		h5Verification,
		/plus\.webview|collectAndroidWebRtcIpsInBackground/
	)
	assert.match(verification, /collectAdminAndroidVerificationIps/)
	...
	assert.match(androidVerification, /createWebRtcDiagnosticLogger\(\s*'admin-android-parent',\s*WEBRTC_DIAGNOSTICS_ENABLED\s*\)/)
	...
	assert.doesNotMatch(androidVerification, /RTCPeerConnection|collectBrowserWebRtcIps|globalThis\.crypto|console\./)
	...
	assert.equal(cryptoInterface, ordinaryCryptoInterface)
	assert.equal(cryptoAndroid, ordinaryCryptoAndroid)
