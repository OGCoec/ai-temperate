...
export function validateMailInspectionTextFile(file) {
	...
	if (!Number.isFinite(size) || size < 0 || size > MAX_FILE_BYTES) {
		throw fileError('FILE_TOO_LARGE', 'TXT 文件不能超过 1 MiB。')
	}
}

export function isMailInspectionFilePickerAvailable(runtime = typeof uni === 'undefined' ? null : uni) {
...
function readWithFileReader(file) {
	return new Promise((resolve, reject) => {
		if (typeof FileReader === 'undefined') {
			reject(fileError('FILE_READ_UNAVAILABLE', '当前环境无法读取所选文件。'))
			return
		}
...
function readWithFileSystem(runtime, filePath) {
	return new Promise((resolve, reject) => {
		...
		if (!manager || typeof manager.readFile !== 'function') {
			reject(fileError('FILE_READ_UNAVAILABLE', '当前 Android 版本请使用多行粘贴。'))
			return
		}
		manager.readFile({
			filePath,
...
async function readText(runtime, file) {
	...
	throw fileError('FILE_READ_UNAVAILABLE', '当前环境无法读取所选文件。')
}

export async function chooseMailInspectionTextFile(runtime = typeof uni === 'undefined' ? null : uni) {
	if (!isMailInspectionFilePickerAvailable(runtime)) {
		throw fileError('FILE_PICKER_UNAVAILABLE', '当前运行环境请使用多行粘贴。')
	}
	...
	validateMailInspectionTextFile({ name, size: file.size })
