	...
	<view class="workspace-panel">
		...
		<view class="inspection-shell">
			...
			<admin-feedback-banner
				...
				tone="warning"
				message="安全会话存储当前不可用；任务编号与实时修订号只保留在内存中。"
			/>
			...
			<view class="workspace-grid" :class="`stage-${workflowStage.toLowerCase()}`">
				...
				<mail-inspection-job-progress
					...
					:connection-state="viewState.connectionState"
				/>
			</view>
...
import MailInspectionCredentialInput from '@/components/admin/mail-inspection-credential-input.vue'
import MailInspectionJobProgress from '@/components/admin/mail-inspection-job-progress.vue'
import MailInspectionResultList from '@/components/admin/mail-inspection-result-list.vue'
import MailInspectionRecoveredJobs from '@/components/admin/mail-inspection-recovered-jobs.vue'
import MailInspectionSensitiveCredentials from '@/components/admin/mail-inspection-sensitive-credentials.vue'
...
import {
	...
	MAIL_INSPECTION_MISSING_JOB_MESSAGE
...
export default {
	...
	created() {
		...
		this.controller = createMailInspectionJobController({
			...
			onChange: next => {
				...
				if (previousJobId && !next.jobId
					&& next.message === MAIL_INSPECTION_MISSING_JOB_MESSAGE) {
					this.focusInvalid = false
					this.inputCollapsed = false
					this.bannerMessage = next.message
					this.bannerType = 'info'
				}
				if (['CREATING', 'DISPATCHING', 'SUBMISSION_UNKNOWN', 'SERVICE_UNAVAILABLE', 'AWAITING_CLIENT_RESUBMISSION', 'QUEUED', 'RUNNING', 'AWAITING_ADMIN_RESUME', 'COMPLETED', 'FAILED'].includes(next.state)
	...
	methods: {
		...
		beforeWorkspaceLeave() {
			return !this.creating && !this.resumeBusyJobId
		},
		hasWorkspaceOverlay() {
