...
<script>
	import {
		cancelAdminAndroidWebRtcProbe,
		completeAdminAndroidWebRtcProbe,
		takeAdminAndroidWebRtcProbeConfiguration
	...
	export default {
		...
		onLoad() {
			this.configuration = takeAdminAndroidWebRtcProbeConfiguration()
		...
		onUnload() {
			...
			if (this.configuration && !this.accepted) {
				cancelAdminAndroidWebRtcProbe(this.configuration.nonce)
			}
		},
		methods: {
			handleMessage(event) {
