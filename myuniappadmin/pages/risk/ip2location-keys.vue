<template>
	<admin-page-shell
		...
		current-path="/pages/risk/ip2location-keys"
		kicker="网络风险"
		title="IP 信誉凭据"
		description="管理 IP2Location 调用池。页面只显示脱敏元数据，密钥由后端加密保存。"
		:busy="loading || importing || deleting"
...
export default {
	mixins: [createAdminPageGuardMixin('/pages/risk/ip2location-keys')],
	...
	onLoad() { this.runAfterAdminRouteGuard(() => this.loadKeys()) },
	onShow() {
		this.runAfterAdminRouteGuard(() => this.loadKeys())
	},
	onBackPress() {
	...
	methods: {
		navigateProtected(route) {
			if (route === '/pages/risk/ip2location-keys') return
