...
/**
 * 使用 xAI Responses 联网请求白名单调用 CLIProxyAPI，并复用项目现有 SSE、来源和安全错误解码。
 */
@Service
public final class XaiResponsesWebSearchStreamingStrategy
    ...
    private final XaiResponsesRequestFactory requestFactory;
    ...
    public XaiResponsesWebSearchStreamingStrategy(
        ...
        this.requestFactory = new XaiResponsesRequestFactory(objectMapper);
    ...
    private Flux<AiConversationModelEvent> execute(
        ...
        return client.post()
                ...
                .flatMapSequential(
                        XaiResponsesWebSearchStreamingStrategy::rejectFailureEvent,
                        1,
                        1);
    }
    ...
    private Flux<AiConversationModelEvent> decodeResponse(
        ...
        if (!response.statusCode().is2xxSuccessful()) {
            // 原始错误正文只在固定内存边界内脱敏，异常链不携带未过滤的供应商内容。
        ...
        if (!eventStream) {
            return response.releaseBody().thenMany(Flux.error(
                    protocolFailure("xAI upstream did not return SSE")));
        }
        return Flux.defer(() -> {
    ...
    private static Mono<AiConversationModelEvent> rejectFailureEvent(
        ...
        if (event instanceof AiConversationModelEvent.Failure) {
            return Mono.error(protocolFailure(
                    "xAI Responses stream ended unsuccessfully"));
        }
    ...
    private static <T> Flux<T> enforceTotalDeadline(
        ...
        Mono<Void> deadline = Mono.delay(maximumDuration)
                .then(Mono.error(new TimeoutException(
                        "xAI upstream maximum stream duration exceeded.")));
    ...
    private Throwable mapFailure(Throwable failure) {
        ...
        return new AiConversationException(
                code, message, true, reason, failure);
    }
}
