...
/**
 * 检查 Kiro/AWS 邮件并区分无注册证据、正常邮件和限制证据。
 */
@Component("kiroMailInspectionStrategy")
public final class KiroMailInspectionStrategy
        extends AbstractMailInspectionStrategy {

    public KiroMailInspectionStrategy(
        ...
        super(
                ...
                new KeywordEvidenceMessageMatcher(
                        properties.matchers().kiro().senderKeywords(),
                        properties.matchers().kiro().subjectKeywords(),
                        properties.matchers().kiro().restrictedPhrases()),
    ...
    public MailInspectionType type() {
        return MailInspectionType.KIRO_STATUS;
    }
    ...
    protected MailInspectionResult classify(
        ...
        if (!scan.mailFound()) {
            return businessResult(
                    ...
                    "kiro_registration_evidence_not_found",
                    oauthAttempts,
                    scan,
                    null);
        }
        if (scan.evidencePhrase() != null) {
            return businessResult(
                    credential,
                    MailInspectionResultStatus.KIRO_RESTRICTED_EVIDENCE_FOUND,
        ...
        if (scan.sender() == null && scan.subject() == null) {
            return businessResult(
                    credential,
                    MailInspectionResultStatus.KIRO_UNCLASSIFIED,
                    "kiro_mail_unclassified",
                    oauthAttempts,
                    scan,
                    null);
        }
        return businessResult(
                credential,
                MailInspectionResultStatus.KIRO_REGISTERED_NORMAL,
