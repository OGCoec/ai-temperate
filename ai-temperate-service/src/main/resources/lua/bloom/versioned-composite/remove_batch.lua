...
for receipt_key, group in pairs(receipt_groups) do
    for start = 1, #group.users, 128 do
        ...
        for index = 1, #membership do
            ...
            if tonumber(membership[index]) == 1 then
...
if current_count < removal_count then
    redis.call('HSET', control_key,
            'state', 'DEGRADED', 'degradedReason', 'COUNTER_UNDERFLOW')
    return -4
end
...
for bucket_key, offsets in pairs(deltas) do
    ...
    for start = 1, #locations, 128 do
        ...
        for index = 1, #values do
            ...
            if location.current < location.delta then
                redis.call('HSET', control_key,
                        'state', 'DEGRADED', 'degradedReason', 'COUNTER_UNDERFLOW')
                return -4
            end
...
for receipt_key, group in pairs(receipt_groups) do
    ...
    for start = 1, #selected, 128 do
        ...
        redis.call('SREM', receipt_key, unpack(users))
    end
end
redis.call('HINCRBY', control_key, count_field, -removal_count)
