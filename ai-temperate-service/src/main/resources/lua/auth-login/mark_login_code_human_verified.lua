...
if values[7] == '1' or redis.call('GET', KEYS[2]) ~= challengeHash then return {4} end
redis.call('UNLINK', KEYS[2])
redis.call('HSET', KEYS[1], 'humanVerified', '1')
local renewedExpiresAt = math.min(now + idleTtl, tonumber(values[10]))
redis.call('HSET', KEYS[1], 'expiresAt', renewedExpiresAt)
redis.call('PEXPIRE', KEYS[1], renewedExpiresAt - now)
return {0, values[1], values[2], values[3], values[4], '1', values[8],
