...
## 0. Codex 受控变更、Diff 审查与自动回退规范

本节是所有 Codex 和其他自动化代理执行代码修改时的强制流程。每次修改都必须被视为一次受控变更：先建立基线，再进行最小修改，完成 diff 审查，并在获得验证授权后根据验证结果决定保留或回退。该流程不得覆盖用户在任务开始前已经存在的修改。

### 0.1 变更前建立基线

1. 修改前必须读取并遵守本文件、相关 `README.md`、项目规范和当前任务计划。
2. 修改前必须检查并记录：
   - `git status --short`
   - `git diff --stat`
   - `git diff`
3. 任务开始时的工作区状态定义为“基线状态”。基线状态中的修改全部视为用户修改，禁止覆盖、删除、回退、重新格式化或纳入本次任务。
4. 如果无法区分基线修改与本次修改，必须停止并报告原因，不得继续编辑。
5. 需要较大范围修改时，应该优先使用独立 Git worktree 或独立分支；未通过验证前，不得合并回用户当前工作区。

### 0.2 变更范围与禁止操作

1. 只允许修改完成当前任务所必需的文件，禁止顺手重构、格式化、升级依赖或修改无关代码。
2. 修改前必须说明准备修改的文件、修改目的、预期影响和验证方式。
3. 未经用户明确授权，禁止执行以下破坏性操作：
   - `git reset --hard`
   - `git checkout -- .`
   - `git restore .`
   - `git clean -fd`
4. 未经用户明确授权，禁止提交代码、推送远程仓库、创建远程分支或创建 Pull Request。

### 0.3 Diff 审查与验证

1. 修改完成后必须先执行 `git diff --check`，然后展示并审查完整 diff。
2. Diff 审查必须确认：
   - 没有无关文件变更；
   - 没有覆盖或删除基线修改；
   - 没有提交密码、Token、Secret、AccessKey 或其他敏感信息；
   - 没有违反项目架构、接口、安全和数据一致性规范；
   - 实际修改与任务目标一致。
3. 验证命令必须根据任务风险选择测试、编译、静态检查或其他可重复检查；不得以“代码看起来正确”代替验证。
4. 如果当前任务或本项目阶段规范禁止自动运行测试、编译、打包、安全扫描或外部服务检查，代理必须遵守该限制，并在最终报告中明确列出未执行的验证。
5. 未经用户明确授权，不得因为验证方便而连接生产服务、修改真实数据或执行不可逆外部操作。

### 0.4 验证失败后的自动回退

发生以下任一情况时，本次变更必须标记为失败并停止继续扩展修改：测试失败、编译失败、静态检查失败、diff 包含无关修改、基线被影响、需求无法证明满足，或无法安全区分本次修改与基线修改。

1. 回退只能撤销本次任务产生的修改，不得影响基线状态。
2. 回退完成后必须重新检查：
   - `git status --short`
   - `git diff --stat`
   - `git diff`
3. 回退后的工作区必须与任务开始时的基线状态一致；如果不能证明一致，必须停止并请求人工处理。
4. 无法安全回退时，禁止强行执行破坏性 Git 命令，必须报告阻塞原因和当前文件状态。
5. 回退完成后必须报告失败原因、触发回退的验证命令、受影响文件以及基线恢复结果。

### 0.5 完成条件与最终报告

只有同时满足以下条件，代理才能报告“任务完成”：需求已实现、diff 已审查、授权范围内的验证全部通过、基线修改未受影响、没有无关变更，并且当前工作区状态与报告一致。
