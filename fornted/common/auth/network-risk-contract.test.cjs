...
test('ordinary WebRTC verification is generation-scoped background work and never persists IPs', () => {
	const app = source('App.vue')
	...
	assert.doesNotMatch(http, /await ensureWebRtcVerified\(\)/)
	assert.doesNotMatch(http, /await recoverWebRtc\(error\)/)
	assert.match(http, /observeWebRtcVerificationHeaders/)
	assert.match(http, /void startWebRtcVerificationInBackground/)
