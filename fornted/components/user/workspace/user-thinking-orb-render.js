...
function resolvePreset(state, size) {
	...
	const normalizedSize = Number(size) === 64 ? 64 : 20
...
function paint(ctx, dots, dark, rMin = 0.3) {
	...
	for (const dot of dots) {
		const alpha = dot.a == null ? 1 : dot.a
		if (alpha < 0.02) continue
		const white = Math.min(1, Math.max(0, dot.white))
		const gray = Math.round((dark ? 1 - white : white) * 255)
		ctx.fillStyle = `rgba(${gray},${gray},${gray},${alpha})`
		ctx.beginPath(); ctx.arc(dot.x, dot.y, Math.max(rMin, dot.r), 0, Math.PI * 2); ctx.fill()
	}
}
function paintLines(ctx, lines, dark) {
	for (const line of lines) {
		const alpha = line.a == null ? 1 : line.a
		if (alpha < 0.02) continue
...
function drawOrbits(ctx, size, time, dark, options) {
	const cx = size / 2; const cy = size / 2; const radius = (size / 2) * 0.82
	...
	for (let orbit = 0; orbit < orbitN; orbit++) {
		...
		for (let k = 0; k < ghostN; k++) {
			...
			dots.push({ x, y, z, r: (options.ghostR || 0.9) * scale, white: 0.72, a: (options.ghostA || 0.5) * (0.4 + 0.6 * depth) })
		}
		for (let particle = 0; particle < particles; particle++) {
			const angle = time * particleSpeed + (particle / particles) * 2 * Math.PI + h2 * 6
			const [x, y, z] = project((ux * Math.cos(angle) + vx * Math.sin(angle)) * orbitRadius, (uy * Math.cos(angle) + vy * Math.sin(angle)) * orbitRadius, (uz * Math.cos(angle) + vz * Math.sin(angle)) * orbitRadius)
			const depth = (z / orbitRadius + 1) / 2
			dots.push({ x, y, z, r: ((options.partR || 1.2) + (options.partRDepth || 1.6) * depth) * scale, white: 0.3 - 0.22 * depth })
		}
	}
...
function drawLattice(ctx, size, time, dark, options, mode) {
	...
	const radius = (size / 2) * (mode === 'wave' ? 0.874 : 0.82)
	...
	for (let ring = 0; ring <= rings; ring++) {
		...
		for (let longitudeIndex = 0; longitudeIndex < longitudeCount; longitudeIndex++) {
			...
			dots.push({
				x: px, y: py, z: projectedZ, r: dotRadius * (mode === 'wave' ? (1 + 0.4 * Math.max(0, wave)) : 1) * scale,
...
function drawBraid(ctx, size, time, dark, options) {
	const cx = size / 2; const cy = size / 2; const radius = (size / 2) * 0.76; const project = makeProj(time * 0.4, 0.3, cx, cy, 1); const scale = radiusScale(size, options.rsPow || 0.6); const dots = []; const ghostCount = options.ghostN || 150
	for (let i = 0; i < ghostCount; i++) { const direction = fibDir(i, ghostCount); const [x, y, z] = project(direction[0] * radius, direction[1] * radius, direction[2] * radius); const depth = (z / radius + 1) / 2; dots.push({ x, y, z, r: 0.8 * scale, white: 0.78, a: 0.1 + 0.22 * depth }) }
	...
	for (let strand = 0; strand < 3; strand++) {
		...
		for (let i = 0; i < strandCount; i++) {
			...
			dots.push({ x, y, z, r: ((options.rBase || 1.2) + (options.rDepth || 1.8) * depth) * scale, white: 0.55 - 0.45 * depth, a: endFade * (0.45 + 0.55 * depth) })
		}
	}
	paint(ctx, dots, dark, options.rMin)
}
...
function drawWeb(ctx, size, time, dark, options) {
	...
	const lines = []; const dots = []; for (let i = 0; i < nodeCount; i++) { for (let j = i + 1; j < nodeCount; j++) { const dx = nodes[i][0] - nodes[j][0]; const dy = nodes[i][1] - nodes[j][1]; const dz = nodes[i][2] - nodes[j][2]; const distance = Math.sqrt(dx * dx + dy * dy + dz * dz); if (distance >= threshold) continue; const [x1, y1, z1] = project(nodes[i][0], nodes[i][1], nodes[i][2]); const [x2, y2, z2] = project(nodes[j][0], nodes[j][1], nodes[j][2]); const depth = ((z1 + z2) / 2 + 1) / 2; lines.push({ x1, y1, x2, y2, white: 0.42, a: (1 - distance / threshold) * (0.3 + 0.55 * depth), w: Math.max(0.6, (options.lineW || 0.8) * scale) }) } }
	for (let i = 0; i < nodeCount; i++) { const [x, y, z] = project(nodes[i][0], nodes[i][1], nodes[i][2]); const depth = (z + 1) / 2; const pulse = 1 + 0.25 * Math.sin(time * 1.4 + i * 2.7); dots.push({ x, y, z, r: ((options.nodeR || 1.4) + (options.nodeRDepth || 1.8) * depth) * pulse * scale, white: 0.55 - 0.45 * depth }) }
	...
	paintLines(ctx, lines, dark); paint(ctx, dots, dark, options.rMin)
}
...
function drawMorph(ctx, size, time, dark, options) {
	...
	const count = Math.max(6, Math.round(34 * (options.iconD || 1))); const radius = (options.rDot || 0.021) * 1.35 * spread; const pulse = 1 + 0.02 * Math.sin(local * 3.1); const dots = []; const center = size / 2; let segmentIndex = 0; let accumulated = 0
	for (let i = 0; i < count; i++) { const target = (i / count) * total; while (accumulated + lengths[segmentIndex] < target && segmentIndex < sampleCount - 1) { accumulated += lengths[segmentIndex]; segmentIndex++ } const a = points[segmentIndex]; const b = points[(segmentIndex + 1) % sampleCount]; const progress = lengths[segmentIndex] ? Math.min(1, (target - accumulated) / lengths[segmentIndex]) : 0; dots.push({ x: center + (a[0] + (b[0] - a[0]) * progress) * pulse * size, y: center + (a[1] + (b[1] - a[1]) * progress) * pulse * size, z: 0, r: Math.max(0.35, radius * size), white: 0.1 }) }
	paint(ctx, dots, dark, options.rMin)
}
...
export default {
	...
	methods: {
		...
		connectCanvas() {
			if (!this.root) return
			const canvas = this.root.querySelector?.('canvas') || null
			if (!canvas || canvas === this.canvas) return
			this.canvas = canvas
			this.context = canvas.getContext?.('2d') || null
			this.dpr = Math.min(2, Number(globalThis.devicePixelRatio || 1))
		...
		configureCanvas(size) {
			...
			const pixelSize = Math.round(size * this.dpr)
			const cssSize = `${size}px`
			// Resizing a canvas clears its bitmap; only do it when its render metrics change.
			if (this.canvas.width !== pixelSize) this.canvas.width = pixelSize
			if (this.canvas.height !== pixelSize) this.canvas.height = pixelSize
			if (this.canvas.style.width !== cssSize) this.canvas.style.width = cssSize
			if (this.canvas.style.height !== cssSize) this.canvas.style.height = cssSize
		},
		draw(time) {
			if (!this.canvas || !this.context || !this.config) return
			const size = Number(this.config.size) === 64 ? 64 : 20
			const preset = resolvePreset(this.config.state, size)
			this.configureCanvas(size)
			this.context.setTransform(this.dpr, 0, 0, this.dpr, 0, 0)
			this.context.clearRect(0, 0, size, size)
			MODE_DRAWS[preset.mode](this.context, size, time, this.config.dark !== false, preset.options)
		},
		restart() {
			this.stop()
			...
			const size = Number(this.config.size) === 64 ? 64 : 20
			const presetSpeed = resolvePreset(this.config.state, size).speed
