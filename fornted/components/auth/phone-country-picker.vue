	...
	<view class="country-field">
		...
		>
			<view class="country-main">
				<template v-if="currentCountry">
					...
					<view v-else class="flag-fallback" aria-hidden="true">
						<uni-icons type="map" size="18" color="#8b9690" />
					</view>
					<text class="country-name">{{ currentCountry.name }}</text>
				</template>
				<template v-else>
					<uni-icons class="country-neutral-icon" type="map" size="20" color="#8b9690" aria-hidden="true" />
			...
			<view class="country-meta" aria-hidden="true">
				...
				<uni-icons type="bottom" size="16" color="#8b9690" />
			</view>
		</button>
			...
			>
				<view class="sheet-header">
					...
					<button class="sheet-close" type="button" aria-label="关闭国家选择" @click="closePicker">
						<uni-icons type="closeempty" size="22" color="#f3f5f4" aria-hidden="true" />
					</button>
				</view>
				<uni-search-bar
					v-model="keyword"
				...
				>
					...
					>
						...
						<view class="country-meta" aria-hidden="true">
							...
							<uni-icons v-if="currentCountry && country.id === currentCountry.id" type="checkmarkempty" size="18" color="#37d39a" />
						</view>
					</button>
				</scroll-view>
			</view>
...
<style lang="scss" scoped>
	...
	.country-trigger:active { border-color: #37d39a; transform: scale(.985); }
	.sheet-close:active { background: #202421; transform: scale(.94); }
	.country-trigger:focus-visible,
	.country-option:focus-visible,
	.sheet-close:focus-visible { outline: none; box-shadow: 0 0 0 3px rgba(55, 211, 154, .18); }
	...
	.country-neutral-copy { color: #8b9690; }
	.country-name { min-width: 0; overflow: hidden; color: #f3f5f4; font-size: 15px; line-height: 1.35; text-overflow: ellipsis; white-space: nowrap; }
	.country-code,
	...
	::v-deep .country-search .uni-searchbar__box { background: #202421 !important; border: 1px solid #303733 !important; }
	::v-deep .country-search .uni-input-input,
	::v-deep .country-search .uni-searchbar__box-search-input { color: #f3f5f4 !important; font-size: 16px !important; }
	...
	.country-option:active { background: #202421; }
	.country-option.selected { background: #193127; }
	.empty-state { padding: 52px 16px; color: #8b9690; font-size: 14px; text-align: center; }
	@media (hover: hover) and (pointer: fine) {
		.country-trigger:hover { background: #1b1f1c; border-color: #455049; }
		.country-option:hover,
		.sheet-close:hover { background: #202421; }
	}
