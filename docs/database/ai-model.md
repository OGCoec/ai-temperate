...
能力代码固定为 `CHAT_COMPLETIONS`、`RESPONSES`、`WEB_SEARCH`，以及图像、音频、视频各自独立的 `*_INPUT`、`*_GENERATION`、`*_EDIT`，合计十二项。每项能力仍保存为一行，管理端创建、详情和 Merge Patch 仍使用 `capabilities` 字符串数组，不增加十二个布尔字段。媒体输入、生成和编辑互不推导：例如模型支持 `IMAGE_GENERATION` 不表示它能读取用户上传的图片；附件输入只检查对应的 `IMAGE_INPUT`、`AUDIO_INPUT` 或 `VIDEO_INPUT`。具体模型支持哪些能力必须由管理员按供应商真实能力显式配置，禁止根据模型名称或厂商自动授予。
...
能力编辑采用整组替换：在同一个 PostgreSQL 本地事务中先完成带版本条件的模型主表更新，再删除该模型旧能力并一次批量插入新能力。Mapper XML 只负责通用枚举到 `VARCHAR` 的批量映射，不硬编码任何具体能力，也不得按能力逐条执行数据库 I/O。这里的能力明细删除不是模型物理删除；任一步失败时主表版本、字段和能力集合全部回滚。未来若经过 ADR 增加模型删除能力，必须先删除 `ai_model_capability`，再删除 `ai_model`，并在同一事务中完成。

`ai_model.context_window_tokens` 与 `ai_model.max_output_tokens` 只保存原始 Token，不保存管理员输入的 K。`1 K` 固定换算为 `1000 Token`，两个字段必须同时为空或同时配置；配置值必须为正数、不超过 `2147483647000`、能被 `1000` 整除，且最大输出不得超过上下文窗口。双空只用于表达尚未配置的历史模型；未配置模型可以继续编辑或停用，但不得启用，也不得进入启用模型缓存。两个容量字段不参与筛选和排序，因此不建立索引。
...
存量环境必须在部署新版应用时执行 `sql/migrations/023_split_ai_model_media_capabilities.sql`。迁移先移除旧 CHECK 约束；当同一模型的新旧输入能力同时存在时删除旧聚合行，再将 `IMAGE`、`AUDIO`、`VIDEO` 分别收敛为 `IMAGE_INPUT`、`AUDIO_INPUT`、`VIDEO_INPUT`，最后恢复十二项白名单。旧能力只在原业务中授权附件输入，因此迁移不会自动增加生成或编辑权限；回滚若需要恢复旧程序，必须先停止新程序写入，并经人工确认后把细分能力按业务决定收敛，不能无损自动回退。

所有模型读取都通过 `ai_model LEFT JOIN ai_model_icon` 得到最终 `icon_url`。HTTP 响应额外返回 `iconPublicId` 供管理端选择。Redis 中的启用模型 AES-256-GCM 快照不保存 K，但会保存两个原始 Token 字段和能力数组；十进制 K 迁移曾将固定 Redis Key 从 v3 升为 v4、快照 Schema Version 从 v4 升为 v5，本次媒体能力拆分继续将 Key 升为 v5、Schema Version 升为 v6。新版本不读取旧 Key；读取到旧 Schema 时按现有失效流程 `UNLINK`，再从 PostgreSQL 重建 v6。图标 URL、启用模型 Token 容量或能力发生修改时，数据库提交成功后刷新整份快照；只修改图标名称或描述不刷新。
