...
所有模型读取都通过 `ai_model LEFT JOIN ai_model_icon` 得到最终 `icon_url`。HTTP 响应额外返回 `iconPublicId` 供管理端选择。Redis 中的启用模型 AES-256-GCM 快照不保存 K，但会保存两个原始 Token 字段；十进制 K 迁移将固定 Redis Key 从 v3 升为 v4，快照 Schema Version 从 v4 升为 v5。新版本不读取旧 Key；读取到旧 Schema 时按现有失效流程 `UNLINK`，再从 PostgreSQL 重建 v5。图标 URL 或启用模型 Token 容量发生修改时，数据库提交成功后刷新整份快照；只修改图标名称或描述不刷新。
