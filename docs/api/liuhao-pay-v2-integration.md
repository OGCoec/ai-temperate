...
> 本项目当前会员 H5 对微信采用后端页面提交：后端只向 `/api/pay/submit` 发送一次 RSA 表单，在禁止自动重定向的客户端上捕获第一跳 `302/303 Location`，再按候选交易号调用 `/api/pay/query` 完成签名、订单归属和金额确认。确认前 `provider_trade_no` 保持 `NULL`，确认后直接绑定真实 `LIUHAO:TRADE:<trade_no>`；支付宝仍使用 `/api/pay/create`。下文保留两类接口的协议资料。
