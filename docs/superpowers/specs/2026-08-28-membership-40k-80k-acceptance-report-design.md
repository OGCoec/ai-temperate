...
最终报告必须直接收录以下四张可复制的 PowerShell 命令卡，不能只描述命令用途。

### 7.1 从 `E-P1` 启动完整八段

这条命令以当前已经完成的 80K 八段 scenario 文件作为精确清理证据，生成新的 Master/Run ID，执行 120 秒 PostgreSQL 稳定性门禁，然后从 `E-P1` 开始执行完整八段：
...
仅当同一 Master 的 `E-P1`、`E-PR`、`E-A1` 已完整通过且 scenario 文件与 PostgreSQL 门禁证据存在时使用：
...
仅当同一 Master 的四个 E 区段均已完整通过时使用。命令同时引用前三段所在 Master Run 和单独完成 `E-AR` 的 Run：
...
这条命令保持用户要求的单行形式，从 `current-80k-run-id.txt` 自动读取当前 Run，每两秒同时显示运行状态和完整 heartbeat JSON：
